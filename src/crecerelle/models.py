...
class DMTranscriptUsageVAE(BetaVAE):
    ...
    def generative_model(self, latent_variable: torch.Tensor, x: torch.Tensor | None) -> Dict[str, torch.Tensor]:
        ...
        # If x is None cross-modal generation of proportions is carried out (necessary for scGETUVI)
        if x is not None:
...
class ZIDMTranscriptUsageVAE(BetaVAE):
    ...
    def generative_model(self, latent_variable: torch.Tensor, x: torch.Tensor | None) -> Dict[str, torch.Tensor]:
        ...
        # If x is None a cross-modal generation of the intron proportion is carried out (necessary in scGETUVI)
        if x is not None:
            generative_model_dict["Number of trials"] = torch.sparse.mm(self.intron_group_summation, x.T).T
            assert torch.all(generative_model_dict["Number of trials"] >= 0).item(), "The number of trials must be non-negative integers"
            assert torch.all(generative_model_dict["Number of trials"] == generative_model_dict["Number of trials"].floor()).item(), "The number of trials must be integers"
...
class SCGETUVI_LEGACY(nn.Module):
    r"""
    ...
    def __init__(
        ...
        super(SCGETUVI_LEGACY, self).__init__()
        ...
        if self.learn_modality_weighting:
            self.weighting_encoder_1 = ScaleEncoder(
                input_dim=input_dim[0],
                output_dim=1,  # Default output_dim for scaling factors
                num_cat_list=None,
                num_hidden_layers=num_hidden_layers[0],
            ...
            self.weighting_encoder_2 = ScaleEncoder(
                input_dim=input_dim[1],
                output_dim=1,  # Default output_dim for scaling factors
                num_cat_list=None,
                num_hidden_layers=num_hidden_layers[1],
    ...
    def variational_posterior(
        ...
        if (x_1_levels is not None) and (x_2_levels is not None):
            if self.learn_modality_weighting:
                ...
                scale_1 = self.weighting_encoder_1(x_1_levels).reshape(-1,1)
                scale_2 = self.weighting_encoder_2(x_2_levels).reshape(-1,1)

                scales = torch.cat((scale_1, scale_2), dim=-1)

                weightings = F.softmax(scales, dim=-1)
    ...
    def forward(
        ...
        variational_posterior_dict = self.variational_posterior((x_1_levels, x_2_levels), eps)
        ...
        # Compute the negative log-likelihood (NLL) for unimodal generation and crossmodal generation per modality
        nll_1_unimodal = self.vae_1.negative_log_likelihood(x_1_counts, generative_model_dict["Modality 1 unimodal"]) # Needs to be more flexible for Gaussian likelihood
        nll_2_unimodal = self.vae_2.negative_log_likelihood(x_2_counts, generative_model_dict["Modality 2 unimodal"]) # Needs to be more flexible for Gaussian likelihood
        nll_1_crossmodal = self.vae_1.negative_log_likelihood(x_1_counts, generative_model_dict["Modality 1 crossmodal"])
        nll_2_crossmodal = self.vae_2.negative_log_likelihood(x_2_counts, generative_model_dict["Modality 2 crossmodal"])

        nll_1 = nll_1_unimodal + nll_2_crossmodal
        nll_2 = nll_2_unimodal + nll_1_crossmodal

        # Compute the Kullback-Leibler divergence (KLD) for the shared latent space and private latent space per modality
        # TO DO: Potential changes to KLD since GMM is approximated as Gaussian with mean and variance of the GMM (potentially wrong atm)

        # Pseudo KLD for shared latent variables
        pseudo_kld_1_shared = self.pseudo_kullback_leibler_divergence(variational_posterior_dict, 1)
        pseudo_kld_2_shared = self.pseudo_kullback_leibler_divergence(variational_posterior_dict, 2)

        # This is the approximated previously used KLD (but mathematically questionable)
        kld_1_shared_approx = self.vae_1.kullback_leibler_divergence(
            variational_posterior_dict["Mixture of experts"]["Latent mean"],
            variational_posterior_dict["Mixture of experts"]["Latent variance"]
        )
        kld_2_shared_approx = self.vae_2.kullback_leibler_divergence(
            variational_posterior_dict["Mixture of experts"]["Latent mean"],
            variational_posterior_dict["Mixture of experts"]["Latent variance"]
        ) # CHECK IF IDENTICAL to kld_1_shared (SHOULD BE!)


        # KLD for private latent variables
        kld_1_private = self.vae_1.kullback_leibler_divergence(
            variational_posterior_dict["Modality 1"]["Private latent mean"],
            variational_posterior_dict["Modality 1"]["Private latent variance"]
        )
        kld_2_private = self.vae_2.kullback_leibler_divergence(
            variational_posterior_dict["Modality 2"]["Private latent mean"],
            variational_posterior_dict["Modality 2"]["Private latent variance"]
        )

        #kld_1 = kld_1_shared_approx + kld_1_private
        #kld_2 = kld_2_shared_approx + kld_2_private

        # Compute negative ELBO per modality
        # TO DO: flexible beta for KL warm-up


        # Named KLD but it is a sum of the pseudo KLD and the KLD of the private latent variable (so value range is not [0, inf])
        kld_1 = self.vae_1.beta * (pseudo_kld_1_shared + kld_1_private)
        kld_2 = self.vae_2.beta * (pseudo_kld_2_shared + kld_2_private)

        elbo_1 = nll_1 + kld_1
        elbo_2 = nll_2 + kld_2

        # Total negative ELBO, total NLL, total KLD
        if self.learn_modality_weighting:
            elbo = (
            ...
            elbo = 0.5 * elbo_1 + 0.5 * elbo_2

        nll = (
                variational_posterior_dict["Mixture of experts"]["Weighting modality 1"] * nll_1
                + variational_posterior_dict["Mixture of experts"]["Weighting modality 2"] * nll_2
        )
        kld = (
                variational_posterior_dict["Mixture of experts"]["Weighting modality 1"] * kld_1
                + variational_posterior_dict["Mixture of experts"]["Weighting modality 2"] * kld_2
        )
